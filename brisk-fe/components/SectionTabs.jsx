...
export default function SectionTabs(props) {
    ...
    const tabArray = tabs.map((tab) => {
        ...
        <li className="mr-2" key={tab}>
            {tab === openTab ? 
                ...
                : <a onClick={handleTabClick} href="#" className="inline-block p-4 rounded-t-lg hover:bg-slate-100">{tab}</a>
            }
        </li>
        )
    });
    ...
    return (
        <ul className="flex grow flex-wrap px-5 text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400">
            {tabArray}
        </ul>
    );
};